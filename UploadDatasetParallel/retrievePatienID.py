...
# python
# File: `UploadDatasetParallel/retrievePatienID.py`
import os
import csv
import logging
...
def get_patient_id(datasets):
    """
    Write patientId.csv for valid dataset folders in `datasets`.
    Skips empty or non-existent entries and raises FileNotFoundError
    if no valid dataset folders are provided.
    """
    # normalize and filter dataset paths
    valid_folders = []
    for d in datasets:
        if not d:
            logger.warning("Skipping empty dataset path.")
            continue
        d = os.path.abspath(d)
        if not os.path.isdir(d):
            logger.warning("Skipping non-existent or non-directory path: %s", d)
            continue
        valid_folders.append(d)

    if not valid_folders:
        raise FileNotFoundError(f"No valid dataset folders provided: {datasets}")

    parent_directory = os.path.dirname(os.getcwd())
    patientId_csv_path = os.path.join(parent_directory, "patientId.csv")
    ...
    with open(patientId_csv_path, 'w', newline='') as csvfile:
        ...
        for folder in valid_folders:
            ...
            # Existing logic to extract PatientID from files goes here.
            # Example placeholder: write each filename as PatientID (adjust to real parsing logic).
            for fname in file_list:
                writer.writerow({'Dataset': folder, 'PatientID': fname})
