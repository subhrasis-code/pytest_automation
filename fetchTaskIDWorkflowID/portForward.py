...
def conductorUI_port_forward(kubeconfig_path, local_port, remote_port, NAMESPACE):
    # Set the KUBECONFIG environment variable
    # kubeconfig_path = kubeconfig_folder_path + kubeConfigFile
    os.environ["KUBECONFIG"] = kubeconfig_path
    # Get the name of the conductorUI pod in the specified namespace
    result = subprocess.run(["kubectl", "get", "pods", "-n", NAMESPACE], capture_output=True, text=True, check=True)
    conductorUI_pod = None
    for line in result.stdout.split('\n'):
        if "conductor-ui" in line:
            conductorUI_pod = line.split()[0]
            break

    # Check if a conductorUI pod is found
    if not conductorUI_pod:
        print(f"No conductorUI pod found in the {NAMESPACE} namespace.")
        sys.exit(1)

    # Perform port forwarding to a random local port
    process = subprocess.Popen(["kubectl", "port-forward", conductorUI_pod,  "-n", NAMESPACE, local_port + ":" + remote_port],
                               stdout=subprocess.PIPE)
